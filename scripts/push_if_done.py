...
def git(*args):
    return subprocess.run(["git", *args], cwd=BASE_DIR, check=True, capture_output=True, text=True)
...
def main():
    ...
    if work_file is None:
        print(f"No work file found for current topic: {topic['id']}")
        return

    if not status_is_done(work_file):
        print(f"Current topic not done yet: {topic['id']}")
        return
